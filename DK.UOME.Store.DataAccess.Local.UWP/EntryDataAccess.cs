...
namespace DK.UOME.Store.DataAccess.Local.UWP
{
    ...
    public class EntryDataAccess : IEntryDataAccess
    {
        ...
        async Task<SQLiteAsyncConnection> EnsureDatabase()
        {
            ...
            // Create entry tables if they doesn't exist
            await database.CreateTableAsync<EntryEntity>();
            await database.CreateTableAsync<PinnedEntry>();
        ...
        {
            var database = await EnsureDatabase();
            var pinnedEntries = await database.Table<PinnedEntry>().ToListAsync();
            var pinnedEntryIds = new List<int>(pinnedEntries.Count);

            foreach (var pinnedEntry in pinnedEntries)
            {
                pinnedEntryIds.Add(pinnedEntry.Id);
            }
        ...
        public async Task UnpinEntry(int unpinnedEntryId)
        {
            ...
            var unpinnedEntry = await database.FindAsync<PinnedEntry>(unpinnedEntryId);

            if (unpinnedEntry != null)
            {
                await database.DeleteAsync(unpinnedEntry);
            }
        }
        ...
        public async Task PinEntry(int pinnedEntryId)
        {
            ...
            await database.InsertAsync(new PinnedEntry() { Id = pinnedEntryId });
        }

        public async Task<Entry> GetEntryById(int entryId)
        {
            var database = await EnsureDatabase();
            var entryEntity = await database.FindAsync<EntryEntity>(entryId);

            if (entryEntity != null)
            {
                return (Entry)entryEntity;
            }
